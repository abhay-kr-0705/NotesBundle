...
export default function EditNotePage({ params }: { params: { id: string } }) {
    // Component for editing existing notes
        ...
        const fetchNote = async () => {
            try {
                ...
                if (note.externalUrl) {
                    ...
                    setFormData(prev => ({ ...prev, externalUrl: note.externalUrl }));
                }
            } catch (error) {
    ...
    }, [params.id, router, categories.length]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Validation
        if (fileMode === 'upload' && !formData.fileUrl) {
            alert('Please upload a PDF file');
            return;
        }
        if (fileMode === 'external' && !formData.externalUrl) {
        ...
        const dataToSubmit = {
            ...formData,
            fileUrl: fileMode === 'upload' ? formData.fileUrl : null,
            externalUrl: fileMode === 'external' ? formData.externalUrl : null,
        };
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex bg-secondary p-1 rounded-lg mb-6">
                                    <button
                                        ...
                                        className={`flex-1 py-2 text-sm font-medium rounded-md transition-all ${fileMode === 'upload'
                                    ...
                                    <button
                                        ...
                                        className={`flex-1 py-2 text-sm font-medium rounded-md transition-all ${fileMode === 'external'
                                            ? 'bg-white shadow text-primary'
                                            : 'text-muted-foreground hover:text-foreground'}`}
                                    >
                                        External Link
                                ...
                                {fileMode === 'upload' ? (
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className="input"
                                            required={fileMode === 'external'}
                                        />
                                        <p className="text-xs text-muted-foreground mt-2">
                                            Paste a direct link to the content (e.g., Google Drive, Dropbox).
